...
def send_email_async(recipient, subject, body, attachment_path=None):
...
def generate_certificate():
    ...
    try:
        ...
        # IMPORTANT: Ensure PDF with QR is fully written before sending email
        import time
        if cloudinary_url:
            time.sleep(0.5)  # Wait 500ms for PDF to be written to disk
        
        # Send email in background - ALWAYS send to BCC, optionally to recipient
        ...
        if os.getenv('EMAIL_SENDER'):
            ...
            # Determine recipient: if user provides email, send to both; otherwise just BCC
            if recipient_email and recipient_email.lower() != bcc_email.lower():
                # Send to recipient (BCC will be added automatically by send_email function)
                send_email_async(
                    recipient_email,
                ...
                print(f"   ✉️ Email queued to: {recipient_email} + {bcc_email}")
            elif bcc_email:
                # No recipient provided, send directly to BCC
                send_email_async(
                    bcc_email,
                    f"Battery Health Certificate - {data.get('registration')}",
                    email_body,
                ...
                print(f"   ✉️ Email queued to: {bcc_email}")
